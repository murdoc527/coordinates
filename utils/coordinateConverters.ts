...
const OSGB36 = {
  ...
  lon0: (-2 * Math.PI) / 180, // Longitude of true origin and central meridian (radians)
  N0: -100000, // Northing of true origin (m)
  E0: 400000, // Easting of true origin (m)
  e2: 0.0066705397616, // Eccentricity squared
};
...
export function toBNG(latitude: number, longitude: number): string {
  try {
    ...
    logCoordinateConversion({ lat: latitude, lon: longitude }, { E, N });

    // Adjust bounds to cover all of UK including offshore areas
    if (E < -100000 || E > 800000 || N < -100000 || N > 1400000) {
      console.log("Coordinates outside extended GB bounds:", { E, N });
...
function calculateM(phi: number): number {
  ...
  const Mb =
    (3 * n + 3 * n2 + (21 / 8) * n3) * Math.sin(phiDiff) * Math.cos(phi + phi0);
  const Mc =
    ((15 / 8) * n2 + (15 / 8) * n3) *
    Math.sin(2 * phiDiff) *
    Math.cos(2 * (phi + phi0));
  const Md =
    (35 / 24) * n3 * Math.sin(3 * phiDiff) * Math.cos(3 * (phi + phi0));

  return OSGB36.b * OSGB36.F0 * (Ma - Mb + Mc - Md);
}

function debugGridSquare(
  letters: string,
  eastingStr: string,
  northingStr: string
) {
...
function transformOSGB36toWGS84(lat: number, lon: number) {
  ...
  if (lat < 51 && lat > 49 && lon < -1 && lon > -6) {
    return {
      latitude: latWGS84 - 0.00002, // Slight southward correction
...
function transformWGS84toOSGB36(
  latWGS84: number,
  lonWGS84: number
...
export function fromBNG(
  ...
  try {
    ...
    // Convert to WGS84 using proj4 string definitions
...
function logCoordinateConversion(
  input: { lat: number; lon: number },
  output: { E: number; N: number }
) {
  console.log("Coordinate conversion:", {
    ...
    isInBounds:
      ...
      output.N >= -100000 &&
      output.N <= 1400000,
    gridSquare: `${Math.floor(output.E / 100000)},${Math.floor(
      output.N / 100000
    )}`,
  });
}
