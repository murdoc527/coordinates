...
import { StyleSheet, View, TextInput, TouchableOpacity, Platform, Clipboard, Share, ScrollView } from 'react-native';
import { ThemedText } from '@/components/ThemedText';
import { ThemedView } from '@/components/ThemedView';
import { fromBNG } from '@/utils/coordinateConverters';
import { FontAwesome } from '@expo/vector-icons';
import { saveToStorage, getFromStorage } from '@/utils/storage';
import { toDDM, toDMS, toBNG, calculateDistance } from '@/utils/coordinateConverters';
...
export default function SearchScreen() {
  ...
  const [distance, setDistance] = useState<{ meters: number; miles: number } | null>(null);
  ...
  useEffect(() => {
    (async () => {
      const saved = await getFromStorage('savedLocations', z.array(SavedLocationSchema));
      const recent = await getFromStorage('recentSearches', z.array(z.string()));
  ...
  const handleSearch = async (input: string) => {
    ...
    try {
      ...
      addToRecent(input);
      await saveToStorage('recentSearches', recentSearches, z.array(z.string()));
    } catch (err) {
  ...
  const parseCoordinates = (input: string) => {
    ...
    try {
      ...
      const gridRefPattern = /^([A-Z]{2})\s*(\d{4,5})(?:\s*,\s*|\s+)(\d{4,5})$|^([A-Z]{2})\s*(\d{8,10})$/;
      ...
      const ddmPattern = /^(\d+)\s+(\d+\.?\d*)[NS][,\s]+(\d+)\s+(\d+\.?\d*)[EW]$/;
      ...
      const dmsPattern = /^(\d+)°\s*(\d+)'\s*(\d+\.?\d*)\"?[NS][,\s]+(\d+)°\s*(\d+)'\s*(\d+\.?\d*)\"?[EW]$/;
      ...
      throw new Error('Could not parse coordinates. Please use one of these formats:\n' +
  ...
  const saveLocation = async () => {
    ...
    await saveToStorage('savedLocations', updatedLocations, z.array(SavedLocationSchema));
      ...
      {editingLocation?.id === location.id ? (
        ...
        <>
          <TouchableOpacity
            ...
            onPress={() => {
              setSearchText(location.originalInput);
              setCoordinates(location.coordinates);
            }}
          >
            <FontAwesome name="bookmark" size={16} color="#007AFF" />
            <View style={styles.locationInfo}>
              <ThemedText style={styles.locationName}>{location.name}</ThemedText>
              <ThemedText style={styles.originalInput}>{location.originalInput}</ThemedText>
            </View>
          </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        {coordinates && (
          <View style={styles.resultContainer}>
            <View style={styles.coordinateBox}>
              ...
              <ThemedText>Lat: {coordinates.latitude.toFixed(6)}°</ThemedText>
              <ThemedText>Long: {coordinates.longitude.toFixed(6)}°</ThemedText>
            </View>

            <View style={styles.coordinateBox}>
              <ThemedText type="subtitle">Degrees Decimal Minutes (DDM)</ThemedText>
              <ThemedText>
                Lat: {`${toDDM(coordinates.latitude, true).degrees}° ${toDDM(coordinates.latitude, true).minutes}.${toDDM(coordinates.latitude, true).decimal.toString().padStart(3, '0')}' ${toDDM(coordinates.latitude, true).direction}`}
              </ThemedText>
              <ThemedText>
                Long: {`${toDDM(coordinates.longitude, false).degrees}° ${toDDM(coordinates.longitude, false).minutes}.${toDDM(coordinates.longitude, false).decimal.toString().padStart(3, '0')}' ${toDDM(coordinates.longitude, false).direction}`}
              </ThemedText>
            </View>

            <View style={styles.coordinateBox}>
              <ThemedText type="subtitle">Degrees Minutes Seconds (DMS)</ThemedText>
              <ThemedText>Lat: {toDMS(coordinates.latitude, true)}</ThemedText>
              <ThemedText>Long: {toDMS(coordinates.longitude, false)}</ThemedText>
            </View>

            <View style={styles.coordinateBox}>
              <ThemedText type="subtitle">British National Grid</ThemedText>
              <ThemedText>{toBNG(coordinates.latitude, coordinates.longitude) || 'Outside UK'}</ThemedText>
            </View>

            <View style={styles.actionButtons}>
              <TouchableOpacity style={styles.actionButton} onPress={() => copyToClipboard(toBNG(coordinates.latitude, coordinates.longitude) || '')}>
                <FontAwesome name="copy" size={24} color="#007AFF" />
              </TouchableOpacity>
        ...
        {savedLocations.length > 0 && (
          <View style={styles.savedContainer}>
            <ThemedText type="subtitle">Saved Locations</ThemedText>
            {savedLocations.map(renderSavedLocation)}
          </View>
        )}
